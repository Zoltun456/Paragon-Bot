    ...
    async def cf(self, ctx, action_or_amount: Optional[str] = None, maybe_user: Optional[discord.Member] = None):
        ...
        if action == "cancel":
            async with coinflip_lock:
                ...
                if not pending:
                    await ctx.reply("No pending coin flip in this channel."); return
                if ctx.author.id != pending["challenger_id"] and not is_control_user_id(ctx.guild, ctx.author.id):
                    await ctx.reply("Only the challenger (or the bot owner) can cancel this coin flip."); return
                coinflip_challenges.pop(chan_id, None)
            await ctx.reply("Coin flip canceled."); return
        ...
        if action == "accept":
            async with coinflip_lock:
                ...
                if time.monotonic() - pending["created"] > CF_TTL_SECONDS:
                    coinflip_challenges.pop(chan_id, None)
                    await ctx.reply("That coin flip expired. Ask the challenger to start a new one."); return
                challenger_id = pending["challenger_id"]
                if maybe_user is not None and maybe_user.id != challenger_id:
                    await ctx.reply("That mention doesn't match the pending challenger in this channel."); return
                if ctx.author.id == challenger_id:
                    await ctx.reply("You can’t accept your own challenge."); return
                challenger = ctx.guild.get_member(challenger_id)
                acceptor = ctx.author
                if not challenger:
                    coinflip_challenges.pop(chan_id, None)
                    await ctx.reply("Challenger left the server. Coin flip canceled."); return
                amount = int(pending["amount"])
                chal_xp = _get_user_xp_int(challenger)
                acc_xp  = _get_user_xp_int(acceptor)
                if chal_xp < amount:
                    coinflip_challenges.pop(chan_id, None)
                    await ctx.reply(f"{challenger.display_name} no longer has {amount} XP. Coin flip canceled."); return
                if acc_xp < amount:
                    coinflip_challenges.pop(chan_id, None)
                    await ctx.reply(f"You don’t have {amount} XP to accept this coin flip."); return
                coinflip_challenges.pop(chan_id, None)
        ...
        if cur_xp < amount:
            await ctx.reply(f"You don’t have {amount} XP to bet."); return
        async with coinflip_lock:
            pending = coinflip_challenges.get(chan_id)
            if pending and time.monotonic() - pending["created"] <= CF_TTL_SECONDS:
                await ctx.reply("There’s already a pending coin flip in this channel. Use `!cf accept` or `!cf cancel`."); return
            coinflip_challenges[chan_id] = {"challenger_id": ctx.author.id, "amount": amount, "created": time.monotonic()}
        ...
        await ctx.reply(
            ...
            f"Type `!cf accept` (or `!cf accept @{ctx.author.display_name}`) to take the bet. "
